<?php
/**
 * EasyTransac multiple payments controller.
 *
...
class EasyTransacMultipayModuleFrontController extends ModuleFrontController
{
    /**
    ...
    {
        $this->module->loginit();
        $this->module->debugLog('Start Multiple Payments');

        $this->module->debugLog($this->context->cart->getOrderTotal(true, Cart::BOTH));

        if (!Configuration::get('EASYTRANSAC_MULTIPAY')) {
            Logger::AddLog('EasyTransac:Multiple Payments: controller called but setting is disabled');
            exit;
        }

        if (!$this->context->customer->id || empty(Tools::getValue('Count')) || !is_numeric(Tools::getValue('Count')) || !$this->context->cart->id) {
            exit;
        }

        $countval = (int) Tools::getValue('Count');

        if (!in_array($countval, [2, 3, 4])) {
            exit;
        }
        ...
        // Multiple payments
        $transaction->setMultiplePayments('yes');
        $transaction->setAmount($total);
        $transaction->setMultiplePaymentsRepeat($countval);
        ...
        if (!$response->isSuccess()) {
            $this->module->debugLog('Payment Page multiple payments Request error: ' . $response->getErrorCode() . ' - ' . $response->getErrorMessage());
            ...
            $next_hop = $response->getContent()->getPageUrl();
            echo json_encode([
                'error' => 'no',
